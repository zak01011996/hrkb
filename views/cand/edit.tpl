...
<form method="post" action="{{ $url }}" enctype="multipart/form-data">
 ...
 <div class="form-group{{if .ErrImg}} has-error{{end}}">
	<div>
        	<img width="300" src="/{{if .cand.Img.Valid}}{{.cand.Img.String}}{{else}}static/img/noavatar.png{{end}}"> 
	</div>
  ...
  <div class="form-group{{if .errEmail}} has-error{{end}}">
	...
	<input type="email" name="email" id="email" class="form-control" placeholder="{{T "email"}}" value="{{.cand.Email}}"> </div> <div class="form-group">
  ...
  <div class="form-group{{if .errMarried}} has-error{{end}}">
   ...
   <div class="form-group{{if .errDep}} has-error{{end}}">
	...
	{{range $dep := .deps}}
		<option value="{{$dep.Id}}" {{if eq $.cand.Id $dep.Id }} selected {{end}} >{{$dep.Title}}</option>
	{{end}}
	</select>
  </div>  
